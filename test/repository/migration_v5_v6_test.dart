...
void main() {
  ...
  group('Database Migration v5 -> v6', () {
    ...
    test('v5 -> latest (v6) verification', () async {
      ...
      final dbV5 =
          await openDatabase(dbPath, version: 5, onCreate: (db, version) async {
        await db.execute(
      ...
      // Test CHECK constraint (loai)
      expect(
          () => db.execute(
              "INSERT INTO buoi_hoc (id_lop, ngay, gio_bat_dau, gio_ket_thuc, loai, created_at, updated_at) VALUES (1, '2026-09-02', '10:00', '11:00', 'INVALID', 'now', 'now')"),
          throwsA(isA<DatabaseException>()));

      // Test CHECK constraint (trang_thai)
      expect(
          () => db.execute(
              "INSERT INTO buoi_hoc (id_lop, ngay, gio_bat_dau, gio_ket_thuc, loai, trang_thai, created_at, updated_at) VALUES (1, '2026-09-02', '10:00', '11:00', 'CHINH', 'INVALID', 'now', 'now')"),
          throwsA(isA<DatabaseException>()));
