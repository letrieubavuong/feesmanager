...
void main() {
  ...
  group('Phase 13A Localization Unit/Widget Tests', () {
    test(
      'AppFormatter formats currency, dates, months and weekdays per locale',
      () {
        final date = DateTime(2026, 10, 15);

        // Vietnamese formatting
        expect(
          AppFormatter.formatCurrency(1000000, locale: 'vi'),
          contains('1.000.000'),
        );
        expect(
          AppFormatter.formatDate(date, locale: 'vi'),
          equals('15/10/2026'),
        );
        expect(
          AppFormatter.formatMonth('2026-10', locale: 'vi'),
          equals('Tháng 10/2026'),
        );
        ...
        // English formatting
        expect(
          AppFormatter.formatCurrency(1000000, locale: 'en'),
          contains('1,000,000'),
        );
        expect(
          AppFormatter.formatDate(date, locale: 'en'),
          equals('Oct 15, 2026'),
        );
        expect(
          AppFormatter.formatMonth('2026-10', locale: 'en'),
          equals('October 2026'),
        );
    ...
    testWidgets('LocaleController switches locale and persists preference', (
      tester,
    ) async {
    ...
    testWidgets('English locale renders English UI labels in SettingsPage', (
      tester,
    ) async {
        ...
        ProviderScope(
          overrides: [
            localeControllerProvider.overrideWith(
              () => MockLocaleController(AppLocaleMode.en),
            ),
          ],
