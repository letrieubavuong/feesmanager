...
void main() {
  ...
  group('Phase 13A Dirty Form Protection Widget Tests', () {
    ...
    testWidgets('Dirty StudentFormPage prompts warning when navigating via Global Menu; Cancel keeps form and entered text', (tester) async {
      ...
      await tester.enterText(find.byType(TextFormField).first, 'Học Sinh Dirty Test');
      await tester.pumpAndSettle();
      ...
      expect(container.read(navigationControllerProvider), equals(AppDestinationId.home));
    });

    testWidgets('Dirty StudentFormPage prompt allows Discarding changes when leaving via Global Menu', (tester) async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
