...
void main() {
  ...
  setUp(() async {
    ...
    service = ScheduleDomainService(scheduleRepo, AssignmentRepository(db), membershipService, classService, studentService);
  });
  ...
  group('ScheduleService - Assignment Management', () {
    test('valid assignment', () async {
      final sId = await studentRepo.create(Student(id: 1, hoTen: 'Student A', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      final cId = await classRepo.create(ClassEntity(id: 1, tenLop: 'Class A', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      final lId = await scheduleRepo.create(ClassSchedule(id: 1, idLop: cId, thuTrongTuan: 1, gioBatDau: '17:30', gioKetThuc: '19:00', hieuLucTu: '2026-09-01', createdAt: DateTime.now(), updatedAt: DateTime.now()));

      await membershipService.enrollStudent(studentId: sId, classId: cId, joinDate: DateTime(2026, 9, 1));
    ...
    test('reject assignment outside membership boundary', () async {
      ...
      await membershipService.enrollStudent(studentId: sId, classId: cId, joinDate: DateTime(2026, 9, 1));
      await membershipService.leaveClass(studentId: sId, classId: cId, endDate: DateTime(2026, 9, 30));
      
      expect(() => service.assignStudent(
        studentId: sId,
        classId: cId,
        scheduleId: lId,
        startDate: DateTime(2026, 10, 1),
      ), throwsA(isA<Exception>().having((e) => e.toString(), 'message', contains('Membership'))));
    });
    ...
    test('reject open-ended assignment with finite membership', () async {
      ...
      expect(() => service.assignStudent(
        ...
        startDate: DateTime(2026, 9, 10),
        endDate: null,
      ), throwsA(isA<Exception>().having((e) => e.toString(), 'message', contains('không có ngày kết thúc'))));
    });

    test('atomic shift change rollback on conflict with OTHER class', () async {
      final sId = await studentRepo.create(Student(id: 1, hoTen: 'Student A', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      final c1Id = await classRepo.create(ClassEntity(id: 1, tenLop: 'Class A', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      final c2Id = await classRepo.create(ClassEntity(id: 2, tenLop: 'Class B', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      
      // Class A - Schedule 1: Mon 17:30
      final l1Id = await scheduleRepo.create(ClassSchedule(id: 1, idLop: c1Id, thuTrongTuan: 1, gioBatDau: '17:30', gioKetThuc: '19:00', hieuLucTu: '2026-09-01', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      // Class A - Schedule 2: Mon 19:00
      final l2Id = await scheduleRepo.create(ClassSchedule(id: 2, idLop: c1Id, thuTrongTuan: 1, gioBatDau: '19:00', gioKetThuc: '20:30', hieuLucTu: '2026-09-01', createdAt: DateTime.now(), updatedAt: DateTime.now()));
      // Class B - Schedule 3: Mon 18:30-20:00 (Conflict with Schedule 2)
      final l3Id = await scheduleRepo.create(ClassSchedule(id: 3, idLop: c2Id, thuTrongTuan: 1, gioBatDau: '18:30', gioKetThuc: '20:00', hieuLucTu: '2026-09-01', createdAt: DateTime.now(), updatedAt: DateTime.now()));

      await membershipService.enrollStudent(studentId: sId, classId: c1Id, joinDate: DateTime(2026, 9, 1));
      await membershipService.enrollStudent(studentId: sId, classId: c2Id, joinDate: DateTime(2026, 9, 1));
      
      // Assign student to Class B Schedule 3 (18:30-20:00)
      await service.assignStudent(studentId: sId, classId: c2Id, scheduleId: l3Id, startDate: DateTime(2026, 9, 1));

      // Assign student to Class A Schedule 1 (17:30-19:00)
      await service.assignStudent(studentId: sId, classId: c1Id, scheduleId: l1Id, startDate: DateTime(2026, 9, 1));
      final assignments = await service.getAssignmentsForStudent(sId);
      final oldAssignmentId = assignments.firstWhere((a) => a.idLichHoc == l1Id).id!;
